...
class Query {
        ...
        function main() {
            ...
            request({
                ...
                if (res.statusCode !== 200) {
                    console.error(`statusCode was ${res.statusCode}. body: ${JSON.stringify(res.body)}`);
    ...
    set(options = {}, cb) {
        ...
        function main() {
            if (!options.bound) return cb(new Error('options.bound required'));
            if (!options.geom) return cb(new Error('options.geom required'));
            ...
            request({
                ...
                body: {
                    type: 'Feature',
                    properties: { },
                    geometry: options.geom.type === 'Feature' ? options.geom.geometry : options.geom
                },
                auth: self.api.user
            }, (err, res) => {
                if (err) return cb(err);
